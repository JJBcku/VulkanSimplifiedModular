...
module VulkanSimplifiers.CommandPoolQFGroupList.Internal;

CommandPoolQFGroupListInternal::CommandPoolQFGroupListInternal(const LogicalDeviceCoreInternal& deviceCore, const DeviceRenderPassDataInternal& deviceRenderPassData,
	...
	const ImageDataListInternal& imageList, WindowListInternal& windowList, const CommandPoolQFGroupListCreationData& creationData, size_t queueFamily) : _deviceCore(deviceCore),
	...
	_imageList(imageList), _windowList(windowList), _device(deviceCore.GetDevice()), _queueFamily(queueFamily),
	_noIndividualResetCommandPoolList(creationData.noIndividualResetCommandPoolListInitialReservation),
...
IDObject<std::unique_ptr<NIRCommandPoolInternal>> CommandPoolQFGroupListInternal::AddCommandPoolWithoutIndividualReset(bool frequentlyRedoneBuffers, size_t queueID,
...
IDObject<std::unique_ptr<IRCommandPoolInternal>> CommandPoolQFGroupListInternal::AddCommandPoolWithIndividualReset(bool frequentlyRedoneBuffers, size_t queueID,
	size_t primaryBufferListInitialCapacity, size_t secondaryBufferListInitialCapacity, size_t addOnReserve)
{
	VkCommandPoolCreateInfo createInfo{};
	createInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
...
NIRCommandPoolInternal& CommandPoolQFGroupListInternal::GetCommandPoolWithoutIndividualResetSimplifier(IDObject<std::unique_ptr<NIRCommandPoolInternal>> poolID)
{
	return *_noIndividualResetCommandPoolList.GetObject(poolID);
}

IRCommandPoolInternal& CommandPoolQFGroupListInternal::GetCommandPoolWithIndividualResetSimplifier(IDObject<std::unique_ptr<IRCommandPoolInternal>> poolID)
{
	return *_individualResetCommandPoolList.GetObject(poolID);
}

const NIRCommandPoolInternal& CommandPoolQFGroupListInternal::GetCommandPoolWithoutIndividualResetSimplifier(IDObject<std::unique_ptr<NIRCommandPoolInternal>> poolID) const
{
	return *_noIndividualResetCommandPoolList.GetConstObject(poolID);
}

const IRCommandPoolInternal& CommandPoolQFGroupListInternal::GetCommandPoolWithIndividualResetSimplifier(IDObject<std::unique_ptr<IRCommandPoolInternal>> poolID) const
{
	return *_individualResetCommandPoolList.GetConstObject(poolID);
}

void CommandPoolQFGroupListInternal::SubmitBuffers(size_t queueID, const std::vector<CommandBufferSubmitInfo>& submitInfos, std::optional<IDObject<AutoCleanupFence>> fenceID)
{
	VkQueue queue = _deviceCore.GetQueue(queueID);
	std::uint32_t queueFamily = _deviceCore.GetQueueFamily(queueID);
...
VkCommandBuffer CommandPoolQFGroupListInternal::GetCommandBuffer(CommandBufferGenericID bufferID) const
{
...
VkCommandBuffer CommandPoolQFGroupListInternal::GetCommandBuffer(IDObject<std::unique_ptr<NIRCommandPoolInternal>> commandPoolID,
...
VkCommandBuffer CommandPoolQFGroupListInternal::GetCommandBuffer(IDObject<std::unique_ptr<IRCommandPoolInternal>> commandPoolID,
